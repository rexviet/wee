...
const writeUserData = async (email) => {
    ...
    firebase.database().ref('subscribers/' + number).set({
      email: email,
    });
  }
...
const getSubscriberByEmail = async (email) => {
    return firebase.database().ref('subscribers/email').child(email).get();
}
...
const checkEmailExists = async (email) => {
    const snapshot = await getSubscriberByEmail(email);
    return snapshot && snapshot.exists();
}