...
const writeUserData = async (email) => {
    ...
    firebase.database().ref('subscribers/number' + number).set({
      email: email,
    });
    firebase.database().ref('subscribers/email' + number).set({
        number: number,
      });
  }
...
const getSubscriberByEmail = async (email) => {
    const snapshot = await firebase.database().ref('subscribers/email').child(email).get();
